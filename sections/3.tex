\section{Đọc dữ liệu}
$\indent$ Ta đọc dữ liệu thu từ file "All\_GPUs.csv" và in ra 5 dòng đầu tiên tương ứng với dữ liệu của mỗi biến. Việc này giúp chúng ta có cái nhìn tổng quan về cấu trúc và đặc tính của dữ liệu từ đó đưa ra được định hướng sơ lược trước khi đi sâu vào phân tích. Đồng thời, các giá trị như chuỗi rỗng,\verb|\n|- ", "\verb|\n|", hoặc "\verb|\n|Unknown Release Date" trong file CSV được chuyển thành NA trong data.frame, qua đó giúp chuẩn hóa dữ liệu, tránh các giá trị không hợp lệ và hỗ trợ việc phân tích dữ liệu khuyết sau này.
\begin{minted}{r}
GPU = read.csv("All_GPUs.csv",header=TRUE,na.strings = c("","\n-","\n","\nUnknown Release Date"))
head(GPU,5), dim(GPU)
\end{minted}
...
\begin{figure}[!h]
    \centering
    \fbox{\includegraphics[width=0.75\linewidth]{images/Screenshot 2025-07-29 105142.png}}
    \caption{Các biến có trong dữ liệu ban đầu}
    \label{fig:enter-label}
\end{figure}

\textbf{Nhận xét:}
Từ thống kê phía trên, ta thấy bộ dữ liệu bao gồm 3406 quan sát và 34 biến, tuy nhiên lại có rất nhiều biến chứa dữ liệu khuyết dù chỉ mới xuất ra màn hình 5 dòng đầu. Chính vì thế, việc phân tích dữ liệu khuyết và chọn lọc các biến liên tục và biến phân loại phải được thực hiện một cách hợp lý, tạo tiền đề cho các phân tích chuyên sâu.
...
Ta thực hiện in bảng thống kê số lượng và tỷ lệ dữ liệu khuyết của từng biến trong tập dữ liệu bằng các câu lệnh:
\begin{minted}{r}
...
\textbf{Nhận xét:}
Từ đồ thị phía trên, ta nhận thấy tỷ lệ dữ liệu khuyết ở các biến có sự khác biệt và chênh lệch nhau lớn. Do đó, đối với những biến có tỷ lệ dữ liệu khuyết cao, ta sẽ không lựa chọn mà chỉ giữ lại các biến có dữ liệu khuyết tương đối.
...
\vspace{0.5em}
 ...
 \textbf{3. Core\_Speed (Tốc độ lõi cơ bản)}
 ...
 \textbf{4. Memory\_Speed (Tốc độ bộ nhớ)}
 ...
 \textbf{5. Memory\_Type (Loại bộ nhớ)}
 ...
 \textbf{6. ROPs (Raster Operation Pipelines)}
 ...
 \textbf{7. TMUs (Texture Mapping Units)}
 ...
 \textbf{8. Memory\_Bus (Độ rộng bus bộ nhớ)}
 ...
 \textbf{9. Memory (Dung lượng bộ nhớ)}
 ...
 \textbf{10. Process (Quy trình sản xuất)}
 ...
 \textbf{11. Shader (Số lượng đơn vị Shader)}
 ...
 \textbf{12. Pixel\_Rate (Tốc độ điền pixel)}
...
\begin{figure}[!h]
    \centering
    \fbox{\includegraphics[width=1\linewidth]{images/Screenshot 2025-08-06 231813.png}}
...
Hầu hết đều ở dạng character (dạng ký tự) trong đó có cả các biến định lượng. Vì vậy, để có thể xử lý và thực hiện các phép toán trên các biến định lượng ta cần thực hiện chuyển giá trị dạng chuỗi sang giá trị dạng số (numeric). Ta định nghĩa một hàm để tách các đơn vị với chữ số bằng khoảng trắng và chuyển về dạng số thực:
\begin{minted}{r}
helper <- function(x){ 
...
\begin{itemize}
    ...
    Vì giá trị “4096x2160” là mode (giá trị xuất hiện nhiều nhất, chiếm 39\%) nên thay thế các giá trị khuyết bằng giá trị này. Sau đó, nhận thấy rằng các nhóm giá trị 4096x2160 (39\%), 2560x1600 (34\%) chiếm tỉ trọng lớn hơn đáng kể so với các phân loại còn lại vì vậy chúng ta thực hiện gom nhóm: 4096x2160 (39\%), 2560x1600 (34\%), Other - Các phân loại còn lại (26\%):
    \begin{minted}{r}
# Resolution_WxH
...
$\indent$
Loại bỏ trùng lặp là quá trình loại bỏ các bản ghi hoặc giá trị lặp lại trong một tập dữ liệu để đảm bảo rằng mỗi hàng hoặc giá trị chỉ xuất hiện một lần. Mục đích chính là làm sạch dữ liệu, tránh sai lệch trong phân tích hoặc mô hình hóa do các bản ghi trùng lặp gây ra.kể
...
\begin{figure}[h!]
    \centering
    \fbox{\includegraphics[width=0.9\linewidth]{images/Screenshot 2026-05-03 033602.png}}
...
% \begin{figure}[!h]
%     \centering
%     \fbox{\includegraphics[width=1\linewidth]{images/Screenshot 2025-07-28 172050.png}}
% \end{figure}
...
% \begin{figure}[!h]
%     \centering
%     \fbox{\includegraphics[width=1\linewidth]{images/Screenshot 2025-07-28 172704.png}}
% \end{figure}
% \begin{figure}[!h]
%     \centering
%     \fbox{\includegraphics[width=1\linewidth]{images/Screenshot 2025-07-28 172716.png}}
% \end{figure}
...
\section{Loại bỏ Outlier và định nghĩa biến}

$\indent$
Sau khi đã loại bỏ các bản ghi trùng lặp, bước tiếp theo trong quá trình tiền xử lý là phát hiện và loại bỏ các giá trị ngoại lai (outlier). Outlier là những quan sát có giá trị quá lớn hoặc quá nhỏ so với phần lớn dữ liệu, có thể gây ảnh hưởng tiêu cực đến kết quả phân tích và mô hình hóa.

\vspace{0.8em}

\textbf{1. Phương pháp sử dụng: IQR (Interquartile Range)}

\vspace{0.5em}

Phương pháp IQR dựa trên khoảng tứ phân vị, trong đó:
\begin{itemize}
    \item $Q1$: Phân vị thứ 25
    \item $Q3$: Phân vị thứ 75
    \item $IQR = Q3 - Q1$
\end{itemize}
...
Các giá trị nằm ngoài khoảng này sẽ bị loại bỏ.

\vspace{0.8em}

\textbf{2. Định nghĩa các biến}

\vspace{0.5em}

\textbf{Biến số (numerical):}
\begin{itemize}
    \item Core\_Speed, Memory\_Speed
    \item ROPs, TMUs
    \item Memory\_Bus, Memory
    \item Process, Shader
\end{itemize}

\textbf{Biến phân loại (categorical):}
\begin{itemize}
    \item Manufacturer, Memory\_Type, Resolution\_WxH
\end{itemize}

\vspace{0.8em}

\textbf{3. Thực hiện trong R:}
...
for (var in numerical) {
  if (var %in% names(GPU_new)) {
    ...
    GPU_new <- GPU_new[
      GPU_new[[var]] >= lower_bound & 
      GPU_new[[var]] <= upper_bound, ]
  }
}
\end{minted}
...
\begin{itemize}
    \item Số lượng dòng trước khi loại outlier: 2204
    \item Core\_Speed: [531.25 ; 1313.25] $\rightarrow$ còn 2036 dòng
    \item Memory\_Speed: [-212.5 ; 2487.5] $\rightarrow$ còn 2036 dòng
    \item ROPs: [-8 ; 56] $\rightarrow$ còn 1905 dòng
    \item TMUs: [-16 ; 112] $\rightarrow$ còn 1728 dòng
    \item Memory\_Bus: [-64 ; 448] $\rightarrow$ còn 1709 dòng
    \item Memory: [-512 ; 3584] $\rightarrow$ còn 1482 dòng
    \item Process: [10 ; 58] $\rightarrow$ còn 1382 dòng
    \item Shader: [3.31 ; 6.01] $\rightarrow$ còn 1231 dòng
\end{itemize}

\vspace{0.5em}

\textbf{Tổng số dòng sau khi loại outlier: 1231}
...
\begin{figure}[h!]
    \centering
    \fbox{\includegraphics[width=0.9\linewidth]{images/Screenshot 2026-05-03 033555.png}}
    \caption{Hình ảnh đối chiếu trước và sau khi loại ngoại lai}
    \label{fig:enter-label}
\end{figure}
...
Sau khi áp dụng phương pháp IQR để loại bỏ outlier trên từng biến số, số lượng quan sát đã giảm đáng kể từ 2204 xuống còn 1231. Điều này cho thấy tập dữ liệu ban đầu chứa nhiều giá trị ngoại lai có thể ảnh hưởng đến kết quả phân tích.

Việc loại bỏ outlier giúp:
\begin{itemize}
    \item Làm giảm nhiễu trong dữ liệu
    \item Cải thiện độ chính xác của các mô hình thống kê
    \item Tăng tính ổn định và khả năng tổng quát hóa của mô hình
\end{itemize}

Tuy nhiên, việc loại bỏ quá nhiều dữ liệu cũng cần được cân nhắc, vì có thể làm mất đi một số thông tin quan trọng trong các trường hợp đặc biệt.
...
\begin{figure}[!h]
    \centering
    \fbox{\includegraphics[width=1\linewidth]{images/Screenshot 2025-08-05 153233.png}}
\end{figure}
...
\begin{figure}[!h]
    \centering
    \fbox{\includegraphics[width=1\linewidth]{images/Screenshot 2025-08-05 153442.png}}
\end{figure}
...
$\indent$
Cuối cùng, ta kiểm tra lại bằng cách tạo bảng tóm tắt các cột dữ liệu và tính tổng số giá trị thiếu (NA) trong mỗi cột của dataframe GPU\_new\_log sau khi xử lý missing value.
...
\begin{figure}[!h]
    \centering
    \fbox{\includegraphics[width=1\linewidth]{images/Screenshot 2026-05-03 034350.png}}
\end{figure}

Việc tiền xử lý đến đây cũng đã đầy đủ, dữ liệu đã loại bỏ những chỗ bị khuyết cũng như đã được xử lý trùng lặp, ngoại lai và gom cụm trước khi tiến hành thống kê. Sau đây, ta sẽ in ra 10 dòng đầu tiên của dataframe GPU\_new\_log. Điều này giúp ta có cái nhìn tổng quan về dữ liệu bằng cách hiển thị một phần của dataframe.
...
%\begin{figure}[!h]
 %   \centering
%    \fbox{\includegraphics[width=1\linewidth]{images/Screenshot 2025-08-06 233357.png}}
%\end{figure}
